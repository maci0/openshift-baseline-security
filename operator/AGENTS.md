...
`config/crd/bases/`, `config/rbac/`, `api/v1alpha1/zz_generated.deepcopy.go`,
and the CRD copy under `bundle/manifests/` all come from `controller-gen`.
Never hand-edit them: run `make generate manifests` and commit the output. CI
fails on `git diff --exit-code` after that command.

The CSV (`bundle/manifests/*.clusterserviceversion.yaml`) is the exception: it
is hand-maintained. `make bundle` validates it but does not write it.
...
- `verify-csv-rbac`: CSV permissions against `config/rbac/`.
- `verify-bundle-static`: the hand-copied RBAC/Service/ConfigMap/PDB
  manifests in `bundle/` against their `config/` sources.
...
- Tolerate the Console capability being absent (`NoKindMatch`) on every path.
...
- Fuzz any parser of cluster-supplied text (suite labels, scan names, CSV
  versions, timestamps). Corpus files under `testdata/fuzz/` are committed; a
  new one appearing in CI fails the run.
