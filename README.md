...
**Current release: 0.6.1** (OLM channel `alpha`, API `baselinesecurity.openshift.io/v1alpha1`).
...
not part of the published 0.6.1 CSV/image tags until the next version bump
...
spec:
  ...
  sourceType: grpc
  image: quay.io/openshift-baseline-security/baseline-security-operator-catalog:0.6.1
---
...
build needed. Pin the release tag (here `0.6.1`) or use `latest`:
