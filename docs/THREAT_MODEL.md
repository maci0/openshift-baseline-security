...
| 4 | ClusterBaseline patch: waivers, schedule, profiles, catalog source | Authenticated API user → operator | User with `update`/`patch` on `clusterbaselines` (aggregated onto the built-in `admin` ClusterRole) | Score integrity (waive FAILs), scan enablement, `ScanSetting` schedule, optional auto-apply remediations, CO catalog override | CRD MaxItems/MaxLength/Pattern (`clusterbaseline_types.go`); no validating admission webhook. Kubernetes API audit is the attribution trail. |
...
| Elevation of privilege | Batch-apply and auto-apply (confused deputy, above). `baseline-security-admin` aggregates onto `admin` (`user_roles.yaml`): a RoleBinding to `admin` in `openshift-compliance` grants namespaced `patch` on remediations and scans in that namespace (node-reboot writes) without cluster-scoped `ClusterBaseline` access. |
...
| Aggregated viewer / admin ClusterRoles | `operator/config/rbac/user_roles.yaml` | Least-privilege humans vs operator SA |
...
2. **Node reboot without the modal.** `oc patch complianceremediation … --type merge -p '{"spec":{"apply":true}}'` with `baseline-security-admin` (or `admin` in `openshift-compliance`). Path: CO applies; this operator is not in the loop. Same for `spec.remediation.apply: Automatic` on the CR, which the operator copies onto `ScanSetting`.
